...
async function sendEmailOtp(req, res) {
    ...
    if (user?.email) {
        const emailverify = await User.findOneAndUpdate({ email: user?.email }, { emailverifyotp }, { new: true });
        if (emailverify) {
        ...
        if (saveUser) {
            const emailverify = await User.findOneAndUpdate({ email }, { emailverifyotp }, { new: true });
            if (emailverify) {
...
async function verifyOtpEmail(req, res) {
    ...
    try {
        ...
        if (response.emailverifyotp === otp) {
            ...
            return COMMON_SERVICE.apiResponse(res, "otp verified successfully", true, 200);
        }
...
async function registerUser(req, res) {
    ...
    try {
        ...
        if (existingUsername) {
            return COMMON_SERVICE.apiResponse(res, "Username is already taken.", false, 500);
        }
        ...
        if (letStore) {
            ...
            mailTransporter.sendMail(mailDetails, (err, data) => {
                if (err) {
                    return COMMON_SERVICE.apiResponse(res, "failed to send mail", false, 500);
                }
...
async function loginUser(req, res) {
    ...
    try {
        ...
        if (!userlogin) return COMMON_SERVICE.apiResponse(res, "user not found", false, 404);

        const dataToSend = await User.findOne({ email }).select(
            "name email verified usertype favorites school_name phoneno city state country postcode school_address school_class school_medium address country",
        );
        ...
        if (!isMatch) {
            return COMMON_SERVICE.apiResponse(res, "invalid credientials", false, 401);
        }
        return COMMON_SERVICE.apiResponse(res, "login sucessFully", true, 201, { data: dataToSend, token });
    } catch (e) {
        return COMMON_SERVICE.apiResponse(res, "something went wrong", false, 500);
    }
}
...
async function forgetPasswordOtp(req, res) {
    ...
    const updateRes = await User.findOneAndUpdate({ email }, { otp }, { new: true });
    if (updateRes) {
        try {
            const mailTransporter = nodemailer.createTransport({
                service: "gmail",
...
async function resetPassword(req, res) {
    ...
    try {
        ...
        if (response.email === email) {
            ...
            if (update) return COMMON_SERVICE.apiResponse(res, "password updated successfully", true, 200);
...
async function updateUserPersonalData(req, res) {
    const { _id } = req.body;
    const updatePersonalData = await User.findOneAndUpdate({ _id: _id }, req.body, {
        new: true,
    });
    console.log(updatePersonalData);
    try {
        if (updatePersonalData) {
            return COMMON_SERVICE.apiResponse(res, "user updatedata successfully", true, 200);
        } else {
...
async function uploadProfilePhoto(req, res) {
    try {
        ...
        if (maxSize < req.file.size) {
            return COMMON_SERVICE.apiResponse(res, "file size must be less then 5 mb", false, 500);
        }
        ...
        const updateUserProfilePhoto = await User.findOneAndUpdate({ _id: req.body._id }, { profile_picture: imageUrl }, { new: true });
...
async function updateProfilePassword(req, res) {
    ...
    if (!email || !new_password || !old_password) {
        return COMMON_SERVICE.apiResponse(res, "please fill all required data", false, 400);
    }

    const getEmail = await User.findOne({ email });
    if (!getEmail) return COMMON_SERVICE.apiResponse(res, "user not found", false, 404);

    try {
        if (old_password === new_password) {
            return COMMON_SERVICE.apiResponse(res, "old password and new password must be different", false, 400);
        }
        const isMatch = await bcrypt.compare(old_password, getEmail.password);
        if (isMatch) {
            const datasave = await User.findOneAndUpdate({ email }, { password: new_password }, { new: true });
            if (datasave) {
                return COMMON_SERVICE.apiResponse(res, "password updated successfully", true, 200);
            }
            return COMMON_SERVICE.apiResponse(res, "failed to update", true, 200);
        }
        return COMMON_SERVICE.apiResponse(res, "old password is incorrect", false, 400);
    } catch (e) {
        return COMMON_SERVICE.apiResponse(res, "something went worng", false, 500);
    }
}
...
async function addToFav(req, res) {
    try {
        ...
        const storeFavPost = await postsService.getOneLocationPostByIdForFavUnFav({ id: location });
        ...
        if (getOneFav) {
            ...
            if (isFavorite === false) {
                ...
                if (deleteRes) {
                    return COMMON_SERVICE.apiResponse(res, "remove from favorite", false, 200);
                }
        ...
        return COMMON_SERVICE.apiResponse(res, "added to favourite", true, 200, newFavorite);
    } catch (e) {
        return COMMON_SERVICE.apiResponse(res, "something went wrong", false, 500);
    }
}
...
async function getOneFave(data) {
    ...
    try {
        const favData = await FavoriteModel.findOne({ user, location }).populate(["user", "post"]).lean(true);
