...
async function createPost(req, res) {
    ...
    try {
        ...
        const data = req.body;
        const fileName = req.body._id + "_" + Math.random(Math.floor()) * 100000 + "." + req.file.mimetype.split("/")[1];
        ...
        console.log(e);
        return COMMON_SERVICE.apiResponse(res, "comething went wrong", false, 500);
    }
}
...
async function getOneLocationPostByIdForFavUnFav(data) {
    try {
        const { id } = data;
        const response = await PostLocationModel.findById(id).populate("post").lean(true);
        ...
        if (keyword) {
            keywordQuery = {
                $or: [
                    ...
                    // { title: { $regex: keyword, $options: "i" } }, // Case-insensitive regex search for school name
