...
const SavedData = ({ state, descriptors, navigation }) => {
  ...
  const savedSchool = useSelector(
    (state) => state?.FindSchoolScreenReducer?.savedSchool
  );
  ...
  useEffect(() => {
    dispatch(
      FindSchoolScreenAction.fetchSavedSchool({ uid: selectUser?.user?._id })
    );
  }, [navigation]);
  ...
  const filteredSchools = savedSchool?.filter((item) => {
  ...
  return (
    ...
    >
      ...
      >
        <FlatList
          refreshing={!savedSchoolLoader ? true : savedSchoolLoader}
          refreshControl={<RefreshControl onRefresh={onRefresh} />}
          onRefresh={onRefresh}
