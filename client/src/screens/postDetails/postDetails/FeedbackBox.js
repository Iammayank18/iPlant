...
const FeedbackBox = ({ BASIC_DATA }) => {
  ...
  let addreviewdata = useSelector(
    (state) => state?.FindSchoolScreenReducer?.addreviewdata
  );

  const addreviewloader = useSelector(
    (state) => state?.FindSchoolScreenReducer?.addreviewloader
  );
  const reviewdata = useSelector(
    (state) => state?.FindSchoolScreenReducer?.reviewdata
  );

  const reviewloader = useSelector(
    (state) => state?.FindSchoolScreenReducer?.reviewloader
  );
  ...
  const addReview = () => {
    const apiPayload = {
      comment: comment,
      rating: ratingValue,
      post: BASIC_DATA?.school?._id,
      user: selectUser?.user?._id,
      location: BASIC_DATA?._id,
    };
    dispatch(FindSchoolScreenAction.fetchAddReview(apiPayload));
    setRatingValue(0);
    setComment("");
    dispatch(
      FindSchoolScreenAction.fetchReviews({ id: BASIC_DATA?.school?._id })
    );
  };
  ...
  useEffect(() => {
    function hitToast() {
      if (addreviewdata?.msg) {
        ...
        dispatch(FindSchoolScreenAction.setAddReview([]));
      }
    }
    hitToast();
  }, [addreviewdata]);
